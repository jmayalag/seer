####
# Optimize triple crossover parameters
# nFast, nMedium and nSlow
###
...
if (!interactive()) {
  ...
  if (length(args) < 1) {
    message("Usage: Rscript opt_triplecross.R [DIR] [start] [end]")
    stop("Missing args")
  }
...
out_dir <- file.path("./results_paper/backtest_opt/triple_cross")
if (!dir.exists(out_dir))
  dir.create(out_dir, recursive = TRUE)
log_file <- file("triplecross-opt-errors.log", open = "wt")
...
optimize <- function(filename, outputdir) {
  ...
  for (nFast in paramset$.nFast) {
    for (nMedium in paramset$.nMedium) {
        ...
        if (nFast < nMedium & nMedium < nSlow) {
          ...
          try(
            invisible(capture.output(results <- backtest.opt(
              symbols = name,
              strategy = triple_crossover(nFast = nFast, nMedium = nMedium, nSlow = nSlow)
            ))),
            outFile = log_file
          )
          b <- Sys.time()
